...
export default function PricingPage() {
  ...
  const fetchSheetData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/sheets/Pricing`);
      ...
      console.log('Fetched pricing data:', values);
    } catch (error) {
  ...
  const handleCellEdit = async () => {
    ...
    try {
      ...
      console.log('Updating pricing cell:', cell, 'with value:', editValue);

      // Use backend API to update the cell
      const response = await fetch(`${BACKEND_URL}/api/sheets/Pricing/${cell}`, {
        method: 'PUT',
        headers: {
      ...
      console.log('Pricing update response status:', response.status);

      if (!response.ok) {
        const errorText = await response.text();
        console.error('Pricing update error response:', errorText);
        throw new Error(`HTTP error! status: ${response.status} - ${errorText}`);
      }

      const result = await response.json();
      console.log('Pricing update successful:', result);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-xl p-8 mb-8">
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-6">
            <div>
              <h1 className="text-4xl font-bold text-gray-800 mb-2">💰 Pricing Management</h1>
              <p className="text-gray-600 text-lg">Edit and manage your pricing data in real-time</p>
            </div>
          ...
          <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="relative">
              <input
                type="text"
                placeholder="Search in pricing data..."
                value={searchTerm}
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-200">
              ...
              <p className="text-blue-600 text-sm mt-2 flex items-center gap-2">
                ...
                Backend: {BACKEND_URL} | Status: {pricingData.length > 0 ? 'Connected' : 'Connecting...'}
              </p>
            </div>
          </div>
        </div>
        ...
        {isLoading && (
          <div className="text-center py-12">
            ...
            <p className="text-xl text-gray-600 font-medium">Loading pricing data...</p>
          </div>
        )}
